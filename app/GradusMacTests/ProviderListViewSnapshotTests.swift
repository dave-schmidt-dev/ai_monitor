...
@Test func providerListViewReportsPinnedTestTimezone() {
    let expected = "America/New_York"
    #expect(ProcessInfo.processInfo.environment["TZ"] == expected)
    #expect(TimeZone.current.identifier == expected)
